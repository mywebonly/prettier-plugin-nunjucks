...
function createPrefix(text: string): string {
  let hash = 5381;
...
function findNunjucksEnd(text: string, start: number, delimiter: "%}" | "}}"): number {
...
function findRawBlockEnd(text: string, start: number): number {
  let pos = start;
  ...
  while (pos < text.length) {
    ...
    if (getTagName(tag) === "endraw") return tagEnd;
    pos = tagEnd;
  }
 ...
 */
function isOnOwnLine(text: string, match: string, offset: number): boolean {
...
export function restorePlaceholders(text: string, map: Map<string, PlaceholderEntry>, tabWidth: number = 2): string {
  let result = text;
      ...
      if (entry.type === "tag" && getTagName(original) === "raw") {
        ...
        return lines
          .map((line, i) => {
            ...
            if (i === lines.length - 1) return edgeIndent + line.trimStart();
