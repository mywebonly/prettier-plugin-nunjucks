...
function normalizeHtmlClosingTags(text) {
...
function reformatArraySection(arrayText, lineIndent, tabWidth) {
    ...
    const wrapTokens = [{ type: "open_bracket", value: "[" }, ...tokens, { type: "close_bracket", value: "]" }];
...
function getNunjucksTagName(line) {
    const match = line.match(/^\{%[-~]?\s*(\w+)/);
    return match ? match[1] : null;
}
function getOpeningTagName(line) {
    const name = getNunjucksTagName(line);
    if (name === null)
        return null;
    if (name.startsWith("end"))
        return null;
    if (SELF_CLOSING_TAG_NAMES.has(name))
        return null;
    if (/^\{%[-~]?\s*set\s+\w+\s*=/.test(line))
        return null;
    return name;
}
function getClosingTagName(line) {
    const name = getNunjucksTagName(line);
...
function formatBlocks(text, tabWidth) {
    const separatedAdjacentTags = text.replace(/(^|\n)([ \t]*)(\{%[-~]?\s*(?:if|for|block|macro|call|filter|raw|set|end\w+)\b.*?[-~]?%\})(?=\{%[-~]?\s*(?:if|for|block|macro|call|filter|raw|set|end\w+)\b)/g, "$1$2$3\n$2");
    ...
    const result = separatedAdjacentTags.replace(/(\S)[ \t]*(\{%[-~]?\s*(?:endif|endfor|endblock|endmacro|endcall|endfilter|endraw|endset)\b.*?[-~]?%\})/g, (match, beforeChar, tagContent, offset) => {
        if (isInsideHtmlTag(separatedAdjacentTags, offset))
            return match;
        return beforeChar + "\n" + tagContent;
    });
...
function normalizeTextareaExpressions(text, tabWidth) {
 ...
 *   {% if id %}id="{{ id }}"{% endif %}
 */
function collapseSimpleIfBlocks(text, printWidth) {
...
function collapseSimpleOpeningTags(text, printWidth) {
 ...
 */
function breakInlineExpressions(text, tabWidth) {
